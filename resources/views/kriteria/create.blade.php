...
@section('title', 'Tambah Kriteria - Sistem Penilaian Guru')
...
@section('content')
    <div class="mb-6">
        ...
        <h1 class="text-2xl font-bold text-gray-800 mt-2">Tambah Kriteria</h1>
    </div>
        ...
        <form action="{{ route('kriteria.store') }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="nama_kriteria" id="nama_kriteria" value="{{ old('nama_kriteria') }}" required
                ...
                <div>
                    <label for="bobot_persen" class="block text-sm font-medium text-gray-700 mb-1">Bobot (%) <span
                            class="text-red-500">*</span></label>
                    <input type="number" name="bobot_persen" id="bobot_persen" value="{{ old('bobot_persen') }}" required
                        step="0.01" min="0" max="100" placeholder="Contoh: 25.00"
                    ...
                    @error('bobot_persen')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                    <p class="mt-1 text-xs text-gray-500">Total bobot semua kriteria harus 100%</p>
                </div>
            </div>
