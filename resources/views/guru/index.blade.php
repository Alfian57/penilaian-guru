...
@section('title', 'Kelola Guru - Sistem Penilaian Guru')

@section('content')
    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-800">Kelola Guru</h1>
            <p class="text-gray-600">Daftar semua guru yang dinilai</p>
        </div>
    ...
    <div class="bg-white rounded-xl shadow mb-6 p-4">
        <form action="{{ route('guru.index') }}" method="GET" class="flex flex-wrap gap-4">
            ...
            <a href="{{ route('guru.index') }}" class="px-4 py-2 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300">
                Reset
            </a>
        </form>
    </div>
                ...
                <tbody class="divide-y divide-gray-200">
                    @forelse($guru as $index => $g)
                        <tr class="hover:bg-gray-50">
                            <td class="px-6 py-4 text-sm text-gray-600">{{ $guru->firstItem() + $index }}</td>
                            <td class="px-6 py-4">
                                <div class="text-sm font-medium text-gray-800">{{ $g->pengguna->nama_lengkap }}</div>
                            </td>
                            <td class="px-6 py-4 text-sm text-gray-600">{{ $g->nip ?? '-' }}</td>
                            <td class="px-6 py-4 text-sm text-gray-600">{{ $g->jabatan ?? '-' }}</td>
                            <td class="px-6 py-4 text-sm text-gray-600">{{ $g->pengguna->email }}</td>
                            <td class="px-6 py-4">
                                <div class="flex items-center justify-center space-x-2">
                                    <a href="{{ route('guru.show', $g) }}" class="p-2 text-blue-600 hover:bg-blue-50 rounded-lg"
                                        title="Lihat">
